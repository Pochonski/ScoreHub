/**
 * src/application/matches/listMatches.js — Use-cases de listado y consulta de partidos (Fase 8).
 *
 * Reemplazan los métodos de `handlers/matchHandler.js`:
 *   - getPartidosHoy     → createGetPartidosHoy
 *   - getPartidosFecha   → createGetPartidosFecha
 *   - getResultadoEquipo → createGetResultadoEquipo
 *   - getProximosEquipo  → createGetProximosEquipo
 *   - getResultadoVS     → createGetResultadoVS
 *
 * Los formateos se preservan idénticos al handler legacy (mismo emoji,
 * misma puntuación, misma detección de grupo por nombre de stage).
 * `mundialCache` se inyecta como dependencia — sigue siendo un servicio
 * con su TTL propio; cuando se decida migrar a un port propio (Fase 3 o
 * Fase 4) se reemplaza la inyección sin tocar el use-case.
 *
 * Helpers `dateStr` / `fmtDate` / `formatTime` / `getGroupFromMatch` se
 * replican verbatim; son funciones puras sin dependencias externas.
 */
...
function createGetPartidosHoy({ cache, getCompetitionName, logger = console }) {
  if (!cache) throw new Error('createGetPartidosHoy: cache required');

  return async function getPartidosHoy() {
    try {
      const today = dateStr();
      const games = await cache.getWorldCupGames({ date: today });
      if (!games || games.length === 0) {
        const compName = await getCompetitionName();
      ...
      const compNameFb = await getCompetitionName();
...
function createGetPartidosFecha({ cache, logger = console }) {
  if (!cache) throw new Error('createGetPartidosFecha: cache required');

  return async function getPartidosFecha(tipoFecha) {
    ...
    let fecha;
    if (tipoFecha === 'today') fecha = dateCR();
    else if (tipoFecha === 'tomorrow') fecha = dateCR(1);
    else if (tipoFecha === 'day_after') fecha = dateCR(2);
    else if (tipoFecha === 'yesterday') fecha = dateCR(-1);
    else if (tipoFecha === 'day_before') fecha = dateCR(-2);
    else if (/^\d{8}$/.test(tipoFecha)) fecha = tipoFecha;
    else if (/^\d{4}-\d{2}-\d{2}$/.test(tipoFecha)) fecha = tipoFecha.replace(/-/g, '');
    else {
      try {
        const d = new Date(tipoFecha);
        if (!isNaN(d)) fecha = d.toISOString().slice(0, 10).replace(/-/g, '');
...
function createGetResultadoEquipo({ cache, logger = console }) {
  ...
  async function getUpcomingMatchTips(homeTeam, awayTeam) {
    try {
      ...
      const dateStrText = date.toLocaleDateString('es-ES', {
...
function createGetProximosEquipo({ cache, logger = console }) {
  if (!cache) throw new Error('createGetProximosEquipo: cache required');

  return async function getProximosEquipo(equipo, limit = 5) {
    try {
      let teamId, teamName;
      if (typeof equipo === 'string') {
        teamName = equipo;
...
function createGetResultadoVS({ cache, mundialista365, logger = console }) {
  ...
  return async function getResultadoVS(home, away) {
    try {
