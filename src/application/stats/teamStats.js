/**
 * src/application/stats/teamStats.js — Use-cases de stats por equipo (Fase 8).
 *
 * Reemplaza `handlers/statsHandler.js`. Funciones:
 *   - getEstadisticas → createGetEstadisticas
...
function createGetEstadisticas({ cache, getRecentForm, formatMatchLine, logger = console }) {
  if (!cache) throw new Error('createGetEstadisticas: cache required');

  return async function getEstadisticas(parsed) {
    const { tipo, equipo } = parsed;
    if (!equipo) return '�️ Indica el equipo. Ej: "Estadísticas de Brasil"';
    try {
      let teamId, teamName;
      ...
      if (!rawMatches || rawMatches.length === 0) return NO_DATA(teamName);
      ...
      logger.error?.({ err: error }, 'Error getEstadisticas');
...
function createGetGoleadores({ cache, getCompetitionName, competitionId, logger = console }) {
  if (!cache) throw new Error('createGetGoleadores: cache required');
  ...
  return async function getGoleadores(limit = 10) {
    try {
      ...
      if (!topScorers.length) return '⚠️ No hay datos de goleadores disponibles.';
      ...
      logger.error?.({ err: error }, 'Error getGoleadores');
