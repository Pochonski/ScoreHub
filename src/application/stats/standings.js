/**
 * src/application/stats/standings.js — Use-cases de tablas de posiciones (Fase 8).
 *
 * Reemplaza `handlers/tableHandler.js`. Funciones:
 *   - getTabla            → createGetTabla
 *   - getTablaMundial     → createGetTablaMundial
 *   - getTablaGrupoMundial → createGetTablaGrupoMundial
 */

const LIGAS_SOPORTADAS = {
  'mundial': { id: 0, nombre: null },
  'mundial 2026': { id: 0, nombre: null },
  'world cup': { id: 0, nombre: null },
  'wc': { id: 0, nombre: null },
  'copa del mundo': { id: 0, nombre: null },
};
...
function createGetTabla({ cache, getCompetitionName, primaryCompetitionId, formatTabla, formatGroupTable, logger = console }) {
  if (!cache) throw new Error('createGetTabla: cache required');

  async function getTablaMundial(ligaNombre) {
    try {
      const standings = await cache.getWorldCupStandings();
      if (!standings || standings.length === 0) return '⚠️ No hay datos de tabla disponibles.';
      const gruposMap = {};
      ...
      if (grupos.length === 0) return '⚠️ No encontré datos por grupo en la respuesta.';
      const compName = ligaNombre || await getCompetitionName(primaryCompetitionId);
      ...
      logger.error?.({ err: error }, 'Error getTablaMundial');
    ...
    try {
      ...
      for (const standing of standings) {
        ...
        if (grupoLetra === grupoUpper) {
          ...
          if (equipos.length === 0) return `⚠️ No hay datos para el Grupo ${grupoUpper}.`;
          return formatGroupTable(equipos, grupoUpper);
        }
      }
      return `⚠️ Grupo ${grupoUpper} no encontrado.`;
    } catch (error) {
      logger.error?.({ err: error }, 'Error getTablaGrupoMundial');
      return `⚠️ No pude obtener la tabla del Grupo ${grupo}.`;
    }
  }

  return async function getTabla(liga) {
    try {
